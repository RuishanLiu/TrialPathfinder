...
def emulate_trials(cohort, features, drug_treatment, drug_control, name_rules, continuous_col=[], categorical_col=[], ps_method='IPTW', thresh_censor=None, name_DrugName='DrugName', name_StartDate='StartDate', name_OutcomeDate='OutcomeDate', name_LastVisitDate='LastVisitDate', verbose=False):
    '''
    Emulate trial
    ...
    def generate_survival_data(df_raw, covariates_col=None, thresh_censor=None, dateofevent='dateofdeath'): 
        '''
        ...
        if covariates_col is not None:
            df = pd.merge(df, df_raw[[name_PatientID]+covariates_col].copy(), how='left', on=name_PatientID)
        return df
    ...
    covariates_col = continuous_col + categorical_col
    
    data_survival = generate_survival_data(features.copy(), covariates_col=covariates_col, thresh_censor=thresh_censor, dateofevent=name_OutcomeDate) 
    data_survival = data_survival.loc[data_survival['duration_recomputed']!=MISSING]
    
    return data_cox, data_trial = generate_trial_cox(cohort, data_survival, drug_treatment, drug_control, 
                                              name_rules=name_rules, ps_method=ps_method, 
                                              continuous_col=continuous_col, categorical_col=categorical_col,
                                              verbose=verbose)
